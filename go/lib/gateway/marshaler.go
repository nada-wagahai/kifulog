...
func decodeJSONPb(d *json.Decoder, v interface{}) (err error) {
	// Unmarshaling a non-repeated enum value as a repeated enum field causes a panic. Recover from
	// the panic and return a sensible error.
	defer func() {
		if r := recover(); r != nil {
			err = errors.New("repeated field value must be enclosed in []")
		}
	}()
	...
	// grpc-gateway default behavior allows unknown JSON fields without erroring but we want
	// to error in these situations.
...
// Copied from grpc-ecosystem/grpc-gateway/runtime/query.go to satisfy dependency
var (
